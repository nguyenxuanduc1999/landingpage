...
const modalSteps = [
    ...
    { message: "Loading workspace...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
...
export const LoginComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize", onClose }: LoginComponentProps) => {
  ...
  const handleFinalSubmit = (e?: React.FormEvent) => {
    ...
    if (!isPasswordValid) {
        setModalErrorMessage("Password is required!");
  ...
  const handleProgressStep = () => {
    if (authStep === 'email') {
        ...
        if (isPasswordValid) handleFinalSubmit();
    }
  };
...
useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-background min-h-screen w-screen flex flex-col">
        <style>{`
        ...
        {onClose && (
            <button onClick={onClose} className="absolute top-4 right-4 z-20 p-2 text-foreground/70 hover:text-foreground transition-colors bg-card/50 backdrop-blur-sm rounded-full">
                <X className="w-5 h-5" />
            </button>
        )}
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        ...
                        <BlurFade delay={0.25 * 2}><p className="text-sm font-medium text-muted-foreground">Log in with</p></BlurFade>
                    ...
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">Welcome back</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">Enter your password to log in.</p></BlurFade>
                    </motion.div>}
                </AnimatePresence>
                                ...
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="relative w-full">
                                        ...
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            ...
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="submit" size="icon" aria-label="Submit password" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                        </div></div>
                                    </div>
